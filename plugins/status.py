...
class StatusPlugin(Plugin):
    ...
    async def on_message(self, message, cmd):
        ...
        if not isop_user(message.author):
            ...
            self.cdb.log_warn_command("Changing bot status requested by NON-OP %s, FAILED" % (cmd.author_nickdis), message)
        else:
            if cmd.action == "status":
                if len(cmd.args) == 0:
                    ...
                    self.cdb.log_info_command("Change bot's status to %s requested by %s" % (cmd.args[0].lower(), cmd.author_nickdis), message)
                ...
                if len(cmd.args) == 0:
                    self.game = None
                    self.cdb.log_info_command("Erasing bot's game requested by %s" % (cmd.author_nickdis), message)
                else:
                    self.game = discord.Game(name=message.content[6:])
                    self.cdb.log_info_command("Change bot's game requested by %s" % (cmd.author_nickdis), message)
